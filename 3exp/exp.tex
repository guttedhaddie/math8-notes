%PUT IN MUCH MORE COMPOUND INTEREST STUFF FOR NEXT YEAR - ALSO MISSING SOME MORE QUESTIONS OF EXP MODELS FROM HW
...
% 
% We know what solutions to the natural growth model look like because we can sketch them: if the value $y(x)$ is known, the equation tells us the value of its \emph{derivative} $y'(x)=ky(x)$ and thus the \emph{direction} in which the solution curve is travelling.
...
Recall the \emph{exponential laws}:\phantomsection\label{pg:explaws}
\[
...
\]
This again shows that $e$ arises very naturally. It is common in finance to compute compound interest \emph{continuously;} a continuous interest rate of $r$\%\ per year means that an initial balance of $y_0$ becomes
\[
...
\begin{enumerate}\itemsep0pt
  \item The balance of an account can be found easily at any time, without worrying how to compute balances part-way between interest payment dates. 
...
\begin{example}{}{}
	...
	\]
	not the \$107,000 you might have expected! This corresponds to a \emph{simple interest rate} (one payment at the end of the year) of 7.25\%.\footnotemark{}
\end{example}
...
% 	\item Suppose that $\lim\limits_{h\to 0}\frac{a^h-1}h=k$. What is the limit if $a$ is replaced by $a^2$?
...
%   In a sample of 100,000 American women, the number whose height is $x$ (rounded to the nearest 0.1\,in) is approximated by the function
%   \[f(x)=\exp\left(\frac{-(x-63.6)^2}{15.68}\right)\] 
\end{enumerate}
\end{exercises}
...
% 	\item A body with initial temperature $T_0$ is placed in an environment with constant temperature $M<T_0$. Suppose that the temperature $T(t)$ obeys Newton's law of cooling, and that it takes $t_1$ seconds for the body to cool to $\frac{T_0+M}2$. Find the temperature at time $t$ and determine how long it takes for the body to cool to $\frac 13(T_0+2M)$.
	...
	% 	\item For the natural growth model, the growth rate is always increasing. It is maximal at the moment the population hits 200 billion and equals
% 	\[P'(t)=(\ln 1.1)P(t)=200\ln 1.1\approx 19.06\,\text{billion cells per hour}\]
% 	For the logistic model, the maximum growth rate happens when the population is half way to carrying capacity: i.e. at 125 billion cells. We therefore have a maximum growth rate of
% 	\[P'(t)=125^2k=\frac{125^2}{250}\ln\frac{33}{28}=\frac{125}2\ln\frac{33}{28}\approx 10.27\,\text{billion cells per hour}\]
% 	at time $t=\frac{\ln\frac 32}{\ln\frac{33}{28}}\approx 2.47$ hours.

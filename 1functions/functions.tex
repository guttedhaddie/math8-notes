	...
	\begin{minipage}[t]{0.75\linewidth}\vspace{0pt}
	  ...
	  \begin{description}
	  	...
	  	For formulæ whose inputs and outputs are real numbers, two conventions are often observed:
			...
			\]
			\smallskip
			For `calculus functions' like this, the vertical line test ($\ast$) really does involve \textcolor{Green}{vertical lines}; every vertical line intersects the graph in precisely one point.
			\smallbreak
			In the picture, the \textcolor{magenta}{dots} are the graph when the domain is the set $\{-1,0,1,2,3\}$ (as described in the table/arrow-diagram).
	  \end{description} 
 	\end{minipage}
  \hfill
 	\begin{minipage}[t]{0.23\linewidth}\vspace{-5pt}
 	  ...
 	  \medbreak
 		\hfill\includegraphics[alt={the graph of a calculus-style function}]{functions-quad}
 	 \end{minipage}
\end{example}
...
\begin{minipage}[t]{0.65\linewidth}\vspace{-8pt}
	To describe the line through points $A$ and $B$, place a virtual ruler so that \textcolor{Green}{0} corresponds to $A$ and \textcolor{Green}{1} to $B$. \textcolor{Brown}{Slide} the ruler so that $A$ moves to the origin $O$: this amounts to \emph{subtracting} the co-ordinates of $A$ from all points on the line. We obtain a \emph{parallel line} through the origin, with $B$ transformed to $P=B-A$. Putting this together with ($\ast$) results in a parametrized description of the line:
	\begin{align*}
		(x,y) &=A+\textcolor{Green}{t}P
...
\begin{example}{}{quadraticeasy}
	...
	\begin{minipage}[t]{0.69\linewidth}\vspace{-5pt}
		Pay attention to the $x$ terms; $-3x^2+12x=-3(x^2-\textcolor{dkred}{4}x)$. Observe that
		\[
			-3(x-\textcolor{dkred}{2})^2=-3(x^2-\textcolor{dkred}{4}x+4)=-3x^2+12x-12
		\]
		gives most of what we want: note how we \emph{divided the \textcolor{dkred}{$x$-coefficient} by two.} To finish, just tidy everything up,
		\[
			y=(-3x^2+12x-12)+16=-3(x-\textcolor{dkred}{2})^2+\textcolor{Teal}{16}
		\]
	\end{minipage}
	\hfill
	...
	The parabola therefore opens downwards ($-3<0$) with its \textcolor{Green}{vertex}, in this case  the maximum, at $(x,y)=(\textcolor{dkred}{2},\textcolor{Teal}{16})$. Otherwise said, we translate the parabola $y=-3x^2$ to the right by \textcolor{dkred}{2} and up by \textcolor{Teal}{16}.
\end{example}
...
\begin{exercises}{}{}
	...
	\begin{enumerate}\setcounter{enumi}{1}
	  ...
	  \begin{enumerate}
	    ...
	    \qquad\qquad\qquad
	    (b) \ $f(x)=x^4-13x^2+36$ %$(x^2-9)(x^2-4)$
	    \item[(c)] $f(x)=x^3-7x-6$ %(x+2)(x^2-2x-3)
	  \end{enumerate}

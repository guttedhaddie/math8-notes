...
This course aims to help future teachers refresh and reinforce some of the conceptual foundations of grade-school mathematics. Teaching is often a matter of \emph{selection}: choosing examples and explanations suited to the level of your students. To do this effectively, you must understand concepts at a higher level than you teach. The \emph{mathematics} in this course shouldn't be difficult for math majors. Instead this is an opportunity to think about content you might one day \emph{teach}. How would \emph{you} explain the material? How does it connect to other ideas? Can you anticipate student questions, and how would you respond?\medbreak

We start with two motivational problems.\footnotemark
	\begin{enumerate}
	  \item How can we travel across the surface of a cube between two opposite vertices so that the path is as short as possible?\par
	  Should you follow the path indicated? If yes, explain why. If not, how should you find the shortest path?\vspace{-15pt}
	  \begin{center}	
	  ...
	  How should we determine where to place the connection so as to minimize the required length of wire? What information do you need in order to find the connection point?
	\end{enumerate}
	...
	\begin{minipage}[t]{0.75\linewidth}\vspace{0pt}
	  ...
	  \begin{description}
	  	...
	  	For formulæ whose inputs and outputs are real numbers, two conventions are often observed:
		 	\begin{itemize}
		 	  \item The \textcolor{red}{domain} is \emph{implied} to be all values for which the formula makes sense (don't do silly stuff like dividing by zero!).
			...
			\]
			\smallskip
			For `calculus' functions like these, the vertical line test ($\ast$) really does involve \textcolor{Green}{vertical lines}; every vertical line intersects the graph in precisely one point.
			\smallbreak
			In the picture, the \textcolor{orange}{dots} are the graph when the domain is the set $\{-1,0,1,2,3\}$ (as described in the table/arrow-diagram).
	  \end{description} 
 	\end{minipage}
  \hfill
...
Can you think of other ways to represent a function? How might your audience influence your choice? 
	...
	\begin{enumerate}
	  ...
	  \item\begin{enumerate}
	    ...
	    \[
	    	B=\{x\in A:x^2>10\}
	    \]
	    and state it in roster notation.
	  ...
	  \item You should be familiar with the Cartesian plane: $\R^2=\big\{(x,y):x,y\in\R\big\}$. This is the set of ordered pairs, typically thought of as co-ordinates.
	  \begin{enumerate}
	    \item Draw a picture of the set $\big\{(x,\sqrt x):x\ge 0\big\}$. Is this the graph of a function? Explain.
	    \item Draw a picture of the set $\big\{(y^2,y):y\in\R\big\}$. Is this the graph of a function? Explain.
	    \item Draw a picture of the \emph{set} $\{(3,4)\}\times (3,4)$. Be careful to distinguish between an ordered pair and an interval. Consider how you would explain this to a student.
	  \end{enumerate}

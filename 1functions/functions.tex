...
\begin{description}\itemsep0pt
	...
	%You should also be familiar with the \emph{Cartesian plane}: $\R^2=\{(x,y):x,y\in\R\}$. The notation $(3,4)\in\R^2$ here describes the \emph{point} with \emph{co-ordinates} $x=3$, $y=4$; don't confuse this with the \emph{interval} $(3,4)=\{x\in\R:3<x<4\}$, which is a subset of $\R$!
\end{description}
...
The reason we need this language when discussing functions is that the inputs and outputs of a function are \emph{elements of sets}. Here is a \emph{very} formal definition of function.%\enlargethispage{2\baselineskip}

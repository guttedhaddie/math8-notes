...
\begin{example}{}{introfunction}
	...
	\begin{minipage}[t]{0.75\linewidth}\vspace{0pt}
	  ...
	  \begin{description}
	  	...
	  	\item[\normalfont\emph{Graph}] This is the set of ordered pairs $\bigl\{\bigl(x,x^2\bigr):x\in\dom f\bigr\}$. In the context of the formal definition (\ref{defn:function}), \emph{the graph is the function!}\par
	  	For formulæ whose inputs and outputs are real numbers, two conventions are often observed:
		 	\begin{itemize}
		 	  \item The \textcolor{Crimson}{domain} is \emph{implied} to be all values for which the formula makes sense (don't do silly stuff like dividing by zero!).
			...
			\]
			\smallskip
			For `calculus functions' like this, the vertical line test ($\ast$) really does involve \textcolor{Teal}{vertical lines}; every vertical line intersects the graph in precisely one point.
			\smallbreak
			In the picture, the \textcolor{Dpink}{dots} are the graph when the domain is the set $\{-1,0,1,2,3\}$ (as described in the table/arrow-diagram).
	  \end{description} 
 	\end{minipage}
  \hfill
	...
	\begin{minipage}[t]{0.7\linewidth}\vspace{-4pt}
		One way to start is to consider the most obvious point on the graph: plug in $x=0$ to obtain $y=1$ and the point $(0,1)$. Next, consider other values of $x$, for instance integers, and plot these:
			\[
				...
				\end{array}
			\]
			Joining these points, it certainly appears as if the graph is a straight line. At this stage you may have enough to convince a middle-school student.
	\end{minipage}
	\hfill
...
\begin{minipage}[t]{0.65\linewidth}\vspace{-8pt}
	To describe the line through points $A$ and $B$, place a virtual ruler so that \textcolor{Crimson}{0} corresponds to $A$ and \textcolor{Crimson}{1} to $B$. \textcolor{Brown}{Slide} the ruler so that $A$ moves to the origin $O$: this amounts to \emph{subtracting} the co-ordinates of $A$ from all points on the line. We obtain a \emph{parallel line} through the origin, with $B$ transformed to $P=B-A$. Putting this together with ($\ast$) results in a parametrized description of the line:
	\begin{align*}
		(x,y) &=A+\textcolor{Crimson}{t}P
		=A+\textcolor{Crimson}{t}(B-A)\\
		&=(1-\textcolor{Crimson}{t})A+\textcolor{Crimson}{t}B
	\end{align*}
	\end{minipage}
	\hfill
...
The points corresponding to various values of $t$ are \textcolor{Crimson}{marked}.\medbreak
...
\begin{example}{}{quadraticeasy}
	...
	\begin{minipage}[t]{0.69\linewidth}\vspace{-5pt}
		Pay attention to the $x$ terms; $-3x^2+12x=-3(x^2-\textcolor{Crimson}{4}x)$. Observe that
		\[
			-3(x-\textcolor{Crimson}{2})^2=-3(x^2-\textcolor{Crimson}{4}x+4)=-3x^2+12x-12
		\]
		gives most of what we want: note how we \emph{divided the \textcolor{Crimson}{$x$-coefficient} by two.} To finish, just tidy everything up,
		\[
			y=(-3x^2+12x-12)+16=-3(x-\textcolor{Crimson}{2})^2+\textcolor{Dgreen}{16}
		\]
	\end{minipage}
	\hfill
	...
	The parabola therefore opens downwards ($-3<0$) with its vertex, here the maximum, at $(x,y)=(\textcolor{Crimson}{2},\textcolor{Dgreen}{16})$. Otherwise said, we translate the parabola $y=-3x^2$ to the right by \textcolor{Crimson}{2} and up by \textcolor{Dgreen}{16}.
\end{example}
	...
	\begin{description}
		...
		\[
			\begin{array}{c|cccccccc}
				n&1&-1&\textcolor{Crimson}{3}&-3&5&\textcolor{Crimson}{-5}&15&-15\\\hline
				n+2&3&1&\textcolor{Crimson}{5}&-1&7&\textcolor{Crimson}{-3}&17&-13%\\\hline
		...
		\begin{description}
		  ...
		  \item[$p^2=\textcolor{Crimson}{q}(15q-2p)$]\lstsp Since the \textcolor{Crimson}{right side} is a multiple of $q$, so also is the \emph{left.} Since $p,q$ have no common factors, we conclude that $q=1$.
		\end{description}
...
\begin{thm}{Rational Roots}{}
	Suppose $f(x)=\textcolor{Blue}{a_n}x^n+\cdots+\textcolor{Crimson}{a_0}$ has \emph{integer} coefficients where $a_n$ and $a_0$ are non-zero. If $x=\frac pq$ is a rational root in simplest terms, then $\textcolor{Blue}{q}$ divides into $\textcolor{Blue}{a_n}$ and $\textcolor{Crimson}{p}$ into $\textcolor{Crimson}{a_0}$.\par
...
\begin{proof}
	...
	\[
		\underbrace{\textcolor{Blue}{a_np^n}+a_{n-1}p^{n-1}q+\cdots+a_1pq^{n-1}}_{\text{divisible by $\textcolor{Crimson}{p}$}} \hspace{-129pt} \overbrace{\phantom{a_{n-1}p^{n-1}q+\cdots+a_1pq^{n-1}}+\textcolor{Crimson}{a_0q^n}}^{\text{divisible by $\textcolor{Blue}{q}$}}=0
	\]
	By considering the braced terms we see that $\textcolor{Blue}{a_np^n}$ is divisible by $\textcolor{Blue}{q}$ and $\textcolor{Crimson}{a_0q^n}$ by $\textcolor{Crimson}{p}$. Since $p,q$ have no common factors, we obtain the result.
\end{proof}
...
\begin{examples}{}{}
	...
	\begin{enumerate}
		...
		\[
			...
			\begin{array}{c|cccccccc}
				x&1&\textcolor{Crimson}{-1}&3&-3&\frac 12&-\frac 12&\textcolor{Crimson}{\frac 32}&-\frac 32\\\hline
				2x-1&1&\textcolor{Crimson}{-3}&5&-7&0&-2&\textcolor{Crimson}{2}&-4
				%x(2x-1)=3\text{?}&&\checkmark&5&-7&0&-2&\textcolor{red}{2}&-4\\\hline
				%f(x)&-2&\textcolor{red}{0}&12&18&-3&-2&\textcolor{red}{0}&3\\\hline
			\end{array}
		\]
		You may prefer to compute $f(x)$ directly: as in the previous example, since we already know $x$ it is quicker to check whether $x(2x-1)=3$ rather than $f(x)=0$. The two roots are \textcolor{Crimson}{indicated}; it is easily verified that the polynomial can be factorized
		\[
			f(x)=(2x-3)(x+1)
		\]
...
\begin{example}{}{factoreasy}
	...
	\begin{description}
	  ...
	  \begin{enumeratea}
	    \item To create $\textcolor{Crimson}{x^3}$, the first term of the quadratic factor must be $\textcolor{Crimson}{x^2}$ 
	    \[
	    	\textcolor{Crimson}{x^3}\textcolor{Blue}{\,-\,x^2}\textcolor{Green}{\,-\,7x}\textcolor{Purple}{\,+\,10}=(x-2)(\textcolor{Crimson}{x^2}\,+\cdots)=\textcolor{Crimson}{x^3}\textcolor{Blue}{\,-\,2x^2}+\cdots
	    \]
	    ...
	    \[
	    	(x-2)(x^2+\,\textcolor{Green}{x}\,+\cdots)=\textcolor{Crimson}{x^3}\textcolor{Blue}{\,-\,x^2}\,\textcolor{Green}{-\,2x}+\cdots
	    \]
	    ...
	    \[
	    	(x-2)(x^2+x-\textcolor{Purple}{\,5})=\textcolor{Crimson}{x^3}\textcolor{Blue}{\,-\,x^2}\,\textcolor{Green}{-\,7x}\textcolor{Purple}{\,+\,10}
	    \]
...
\begin{exercises}{}{}
	...
	\begin{enumerate}\setcounter{enumi}{1}
	  ...
	  \item Students often follow a heuristic when trying to factorize a polynomial $f(x)=0$: try some small integer values for $x$ until you find a root, then apply long division. For what types of polynomial $f(x)$ will this approach work? When will it fail? Explain.
